...
| `experiments/runs/stm-motion-test-jetson/` (Jetson, remote) | one-second C++ STM smoke test with `vx_cmd=0.02 m/s`; serial loop completed, but applied telemetry remained zero | `candidate-not-evidence; host write path only, actuation unconfirmed` |
