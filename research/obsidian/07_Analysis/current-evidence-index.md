...
| `scripts/python/tools/manual_map.py` | no-ROS keyboard teleoperation, STM-telemetry dead-reckoned 2-D occupancy insertion, and CSV/JSON/PGM/YAML output path; local self-test passed | `software-validated; hardware map run pending; no scan-matching or loop-closure claim` |
