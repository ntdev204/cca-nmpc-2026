...
| `experiments/runs/physical-preflight-20260821-2/sensor_preflight.json` (Jetson, remote) | retry after power-on: direct N10P start/stop at 460800 baud for 5 s received 0 bytes and decoded 0 packets; robot remained stationary | `preflight blocked; inspect N10P power/spin/wiring/baud` |
| `experiments/runs/physical-preflight-20260821-3/sensor_preflight.json` (Jetson, remote) | full-power retry: 49 STM32 samples, 15 Astra-S frames, and 48 non-empty N10P scans with 200--214 decoded points; robot remained disarmed | `sensor gate passed; motion/map trial pending` |
| `experiments/runs/app-motion-030-1s-20260821-2/` | one-second forward body-velocity command at `0.30 m/s` through the no-ROS backend; STM32 telemetry peaked at `0.236 m/s`, zero stop tail and final disarm were observed; application pose stayed at zero and a watchdog-zero event occurred before the motion window | `candidate-not-evidence; commissioning only; pose/watchdog findings must be resolved before mapping` |
