...
void CCA_CAN_SEND(void)
{
	u8 frame[8];
	u8 pwm_mask=0;
	u8 external_faults=0;
	float wheels[4];
	uint16_t battery_mv;
	wheels[0]=MOTOR_A.Encoder;
	wheels[1]=MOTOR_B.Encoder;
	wheels[2]=MOTOR_C.Encoder;
	wheels[3]=MOTOR_D.Encoder;
	if(MOTOR_A.Motor_Pwm!=0)pwm_mask|=1u<<0;
	if(MOTOR_B.Motor_Pwm!=0)pwm_mask|=1u<<1;
	if(MOTOR_C.Motor_Pwm!=0)pwm_mask|=1u<<2;
	if(MOTOR_D.Motor_Pwm!=0)pwm_mask|=1u<<3;
	if(Flag_Stop)external_faults|=1u<<0;
	if(!EN)external_faults|=1u<<1;
	if(robot_mode_check_flag)external_faults|=1u<<2;
	battery_mv=(uint16_t)target_limit_int((int)(Voltage*1000.0f),0,65535);
	CcaTelemetry_EncodeStatus(frame);
	CAN1_Send_Num(CCA_CAN_STATUS_ID,frame);
	CcaTelemetry_EncodeApplied(frame);
	CAN1_Send_Num(CCA_CAN_APPLIED_ID,frame);
	CcaTelemetry_EncodeWheelsAbc(wheels,
		Robot_Parament.WheelDiameter/2.0f,frame);
	CAN1_Send_Num(CCA_CAN_WHEELS_ABC_ID,frame);
	CcaTelemetry_EncodeWheelD(wheels,
		Robot_Parament.WheelDiameter/2.0f,battery_mv,
		pwm_mask,external_faults,frame);
	CAN1_Send_Num(CCA_CAN_WHEEL_D_ID,frame);
}
